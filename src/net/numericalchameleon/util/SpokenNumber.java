 ...
 *
 */
package net.numericalchameleon.util;
...
abstract public class SpokenNumber {
    ...
    protected void checkNumber(String number) throws Exception {
        ...
        if (big.compareTo(MAX_VALUE) > 0) {
            throw new RuntimeException(MessageFormat.format(ERROR_TOOBIG, MAX_DIGITS));
        } else if (big.compareTo(MIN_VALUE) < 0) {
            throw new RuntimeException(MessageFormat.format(ERROR_TOOSMALL, MAX_DIGITS));
        }
    }
